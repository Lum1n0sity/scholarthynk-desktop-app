...
const { fs, flatpickr, WebSocket, ipcRenderer, Store, config, Chart, shell } = require(pathIndex.join(rootPathIndex, 'utils.js'));
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    create_assignment.addEventListener('click', () => {
        ...
        const assignmentData = ({ school: store.get('school'), name: title_assignment, desc: desc_assignment, dueDate: due_date_assignment, students: assignedStudentsArray, subject: assignment_select });
    ...
    update_assignment.addEventListener('click', () => {
        ...
        // * Prepare data for request
        const school = store.get('school');
        const updatedAssignmentData = ({ school: school, title: title, description: desc, dueDate: dueDate, students: students, oldTitle: oldTitle })
        ...
        fetch(`${config.apiUrl}/teacher/review/get-files`, {
            ...
            .then(data => {
                ...
                for (const file of files) {
                    ...
                    tab.addEventListener('click', async () => { document_viewer.innerHTML = ''; tab.classList.add('tab-selected'); await loadFile(file.content, file.name) });
	...
	fetch(`${config.apiUrl}/teacher/get-students`, {
		...
		.then(data => {
			...
			students.forEach(student => {
				...
				const openStudentsGradebook = document.createElement('button');

				openStudentsGradebook.classList.add('grade-book-student');

				openStudentsGradebook.textContent = name;

				openStudentsGradebook.addEventListener('click', async () => { await openGradebook(name) });

				studentsListGradebook.appendChild(openStudentsGradebook);
			})
		})
		.catch(err => {
			console.error('Fetch error: ', err);
	...
	async function openGradebook(name) {
        ...
        background.stlye.display = 'block';
